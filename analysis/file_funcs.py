...
from context import name_dir, script_dir, json_dir, output_dir
...
def dist_from_ridge_fire(ter_cross):
    # use a cross section of terrain to get a 1d vector of distance from ridge high point
    ...
    with open(str(json_dir) + "config.json") as f:
        config = json.load(f)
    dx = config["grid_dimensions"]["dx"]
    sr_x = config["grid_dimensions"]["sr_x"]
    ...
    # create vector of distance
    ridge_dist = np.arange(1, (len(ter_cross)*sr_x)+sr_x+1, 1)
    for ii in range(0, len(ridge_dist)):
        ridge_dist[ii] = (ii - (max_ind[0][0] * sr_x)) * (dx / sr_x)
