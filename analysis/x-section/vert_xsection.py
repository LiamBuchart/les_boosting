"""
    ...
    August 12, 2022
    November 10, 2025 - reconfig for les-boosting
"""
...
## USER INPUTS ##
# options ["]
exp = "TEST-VALLEY-LARGE/" # name of the experiment you want to plot
start = (0, 80)
end = (-1, 80)
...
for ii in range(0, len(wrfin)):
    ...
    axs[0].set_xticks(np.arange(-17000, 19000, 6000))
    axs[1].set_xticks(np.arange(-17000, 19000, 6000))
    ...
    if ii == 5:
        ...
        all_spd = np.dstack((all_spd, UU))
        all_pot = np.dstack((all_pot, t))

        
# make the mean figure
fig, axs = plt.subplots(2, 1, 
                        sharex=True,
                        gridspec_kw={"height_ratios":[5, 1]})

temp_levels = np.arange(270, 370, 2)  # contour lines 
wind_levels = np.arange(-15, 26, 1)
...
plt.savefig(save_path + "potential_speed_mean")
plt.close()
